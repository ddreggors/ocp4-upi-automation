...
* The `bootstrap_vms`, `master_vms`, and `worker_vms` are how you define the nodes.
...
[%hardbreaks%]
Assuming you are able to resolve bootstrap.mycluster.mydomain.com on this machine
Replace the bootstrap hostname with any of the master or worker hostnames
----
ssh -i ~/.ssh/ocp4 core@bootstrap.mycluster.mydomain.com
----
...
* Moving all iso generation tasks into their own role
* Creating plays for adding isos for additional worker nodes (post-install)
